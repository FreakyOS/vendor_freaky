...
- freakygerrit:     A Git wrapper that fetches/pushes patch from/to FreakyOS Gerrit Review.
- freakyrebase:     Rebase a Gerrit change and push it again.
- freakyremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for FreakyOS Github.
...
- repopick:        Utility to fetch changes from FreakyOS Gerrit.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/FreakyOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop org.freaky.device | grep -q "$CUSTOM_BUILD"); then
...
function freakyremote()
{
    ...
    git remote rm freaky 2> /dev/null
    ...
    git remote add freaky https://android.googlesource.com/$PFX$PROJECT
    echo "Remote 'freaky' created"
}
...
function githubremote()
{
    ...
    local REMOTE=$(git config --get remote.freaky.projectname)

    local PROJECT=$(echo $REMOTE | sed -e "s#platform/#android/#g; s#/#_#g")

    git remote add github https://github.com/FreakyOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop org.freaky.device | grep -q "$LINEAGE_BUILD");
    then
...
function freakyrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "FreakyOS Gerrit Rebase Usage: "
        echo "      freakyrebase <path to project> <patch IDs on Gerrit>"
    ...
    cd $dir
    repo=$(git config --get remote.freaky.projectname)
    echo "Starting branch..."
    repo start tmprebase .
    echo "Bringing it up to date..."
    repo sync .
    echo "Fetching change..."
    git fetch "http://freakyos.me/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop org.freaky.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/freaky/build/tools/repopick.py $@
}
